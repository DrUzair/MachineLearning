...
from MyGMM import train_gmm

def prep_data():
    X, Y = make_blobs(cluster_std=1, random_state=110, n_samples=10000, centers=10)
...
if __name__ == '__main__':
    ...
    # means, covs, weights = train_gmm(X, gmm_k=gmm_k)
    ...
    #
    # sum_post_probs = np.sum(
    #     [multivariate_normal(mean=mean, cov=cov).pdf(query_points[0]) for mean, cov in zip(means, covs)])
    # prediction = np.argmax(
    #     np.divide([multivariate_normal(mean=mean, cov=cov).pdf(query_points[0]) for mean, cov in zip(means, covs)],
    #               sum_post_probs))
    # print(prediction)
    # sum_post_probs = np.sum(
    #     [multivariate_normal(mean=mean, cov=cov).pdf(query_points[1]) for mean, cov in zip(means, covs)])
